...
public class MerchantWorld extends World{
    public MerchantWorld(Player player1){
        ...
        setPaintOrder(Star.class, Player.class, GoldNugget.class, Edible.class, Rock.class);
        ...
        for(int i = 2; i<15;i++){
            if (i != 9) { // Don't spawn rock on teleporter
                addObject(new Rock(0,1000000),i,  5);
            }
        }
        for(int i = 0; i<15;i++){
            addObject(new Rock(0,1000000),i,  3);
        }
        for(int i = 0; i<15;i++){
            addObject(new Rock(0,1000000),i,  7);
        }
